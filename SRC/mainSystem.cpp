...
int outputchoice = -1;
#ifdef DLL
#define CS_API_EXPORT __declspec(dllexport)
#else
#define CS_API_EXPORT
#endif

CS_API_EXPORT DEBUGAPI debugInput = NULL;
CS_API_EXPORT DEBUGAPI debugOutput = NULL;
CS_API_EXPORT DEBUGAPI debugEndTurn = NULL;
CS_API_EXPORT DEBUGLOOPAPI debugCall = NULL;
CS_API_EXPORT DEBUGVARAPI debugVar = NULL;
CS_API_EXPORT DEBUGVARAPI debugMark = NULL;
CS_API_EXPORT DEBUGAPI debugMessage = NULL;
CS_API_EXPORT DEBUGAPI debugAction = NULL;
...
void HandlePermanentBuffers(bool init)
{
	if (init)
	{
		...
		*readBuffer = 0;
		...
		ourMainInputBuffer = (char*)mymalloc(fullInputLimit);  // precaution for overflow
#ifndef DLL
		ourMainOutputBuffer = (char*)mymalloc(outputsize);
#else
		ourMainOutputBuffer = (char*)revertBuffer; // for boot call to function on startup potentially
#endif
...
static void ShowMemoryUsage()
{
	...
	*at = 0;
	...
	sprintf(data, (char*)"Free %sMB: dict %s(%uMb) fact %s(%sMb) stack/heap %sMB \r\n\r\n",
		buf2, StdIntOutput(dictfree), (unsigned int)dictFreeMemMB, buf, factmb, buf1);
	if (server) Log(SERVERLOG, "%s", data);
	else (*printer)((char*)"%s", data);
}
